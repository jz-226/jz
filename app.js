...
const caseData = {
  ...
  tutor: { count: '03', type: 'SHIPPED MINI PROGRAM', kicker: 'CASE STUDY / JIANG JI XUEQING BEN', title: '把每节课留下来。', lead: '一个给家教老师使用的轻量教学工作台。围绕错题状态流转，帮助老师快速查看学生情况并沉淀课后信息。', meta: [['我的职责', '需求梳理 · 交互 · 视觉 · 原生开发'], ['核心模块', '学生档案 · 备课 · 错题 · 总结'], ['交付状态', '微信小程序已上线']], sections: [['01 / CONTEXT', '家教工作往往分散在聊天记录、纸张和临时笔记里，课后信息很容易丢失。'], ['02 / STRUCTURE', '首页统计今日课程、待讲错题与周进度，底部导航固定四个最高频入口。'], ['03 / STATE', '错题从“待讲解”到“已讲解”再到“已掌握”，让学习状态有明确的下一步。'], ['04 / COURSE FLOW', '创建课程时填写学生、科目、日期、状态、教学目标、知识点和练习题，让备课信息在课前就结构化。'], ['05 / BUILD', '原生 WXML / WXSS / JS，自定义 tabBar，使用 wx.setStorageSync 实现开箱即用。']], links: [['扫码体验小程序', '#work'], ['观看创建课程录屏', 'video:assets/tutor-create-course.mp4']] },
  wordquest: { count: '04', type: 'SHIPPED H5 GAME', kicker: 'CASE STUDY / WORD QUEST', title: '让背单词，也有一条冒险路线。', lead: '一款发布在 TapTap 的 CET-4 四级背词 RPG。用关卡、战斗和成长系统承接词汇学习，让记忆从一次答题变成持续推进的游戏过程。', meta: [['我的职责', '玩法设计 · 视觉整合 · 前端开发'], ['核心机制', '词汇战斗 · 六重关卡 · 角色成长'], ['交付状态', 'TapTap H5 已上线']], sections: [['01 / CONTEXT', '传统背词容易变成孤立的重复练习，用户很难感受到进度和反馈。'], ['02 / GAME LOOP', '进入关卡 → 通过词汇题战斗 → 获得成长反馈 → 解锁新的区域和挑战。'], ['03 / VISUAL', '用奇幻地图、角色对战和高辨识度答题面板，把学习目标转成可感知的冒险任务。'], ['04 / BUILD', '纯前端实现，使用 HTML、CSS、JavaScript 和本地资源系统，支持离线运行。']], links: [['打开游戏', 'game/index.html']] }
};
...
function initSphere() {
  ...
  const gameRing = new THREE.Mesh(new THREE.TorusGeometry(2.06, .014, 10, 100), new THREE.MeshBasicMaterial({ color: 0xe2bf5a, transparent: true, opacity: .78 })); gameRing.rotation.x = 1.62; gameRing.rotation.y = .42; group.add(gameRing);
  const nodeColors = [0xb8dd54, 0xff7543, 0x35e7ee, 0xe2bf5a];
  const nodes = nodeColors.map((color, index) => { const node = new THREE.Mesh(new THREE.SphereGeometry(.12, 12, 12), new THREE.MeshBasicMaterial({ color })); node.userData.phase = index * (Math.PI * 2 / nodeColors.length); group.add(node); return node; });
