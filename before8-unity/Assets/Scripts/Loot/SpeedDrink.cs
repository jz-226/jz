using UnityEngine;
using Before8AM.Run;
using Before8AM.Collection;
using Before8AM.Mission;   // [0.8.0] 每日任务"搜刮"进度

namespace Before8AM.Loot
{
    /// <summary>
    /// [0.3.0] 加速饮料：**[0.5] 拾取进背包，使用时移速 ×1.4 持续 8 秒**（RunManager.ApplyItemEffect 数值，与旧"拾取即用"一致）。
    /// 照抄 TorchItem 自动拾取模式：Trigger 碰撞 + 旋转漂浮提示 + taken 防重复 + 延迟销毁。
    /// </summary>
    public class SpeedDrink : MonoBehaviour
    {
        public float RotateSpeed = 90f;
        public float BobSpeed = 2.5f;
        public float BobHeight = 0.12f;

        bool taken;
        Transform _t;
        Vector3 basePos;
        bool basePosInit;   // [0.4.3] 首帧 Update 捕获随机化后位置（LayoutRandomizer.Start 先移动物体）

        void Awake()
        {
            _t = transform;
        }

        void Update()
        {
            if (taken) return;
            if (!basePosInit) { basePos = _t.position; basePosInit = true; }   // [0.4.3] 惰性初始化（随机化后位置）
            _t.Rotate(0f, RotateSpeed * Time.deltaTime, 0f);
            _t.position = basePos + Vector3.up * (Mathf.Sin(Time.time * BobSpeed) * BobHeight);
        }

        void OnTriggerEnter(Collider other)
        {
            if (taken) return;
            if (!other.CompareTag("Player")) return;
            taken = true;
            CollectionSystem.Unlock(CollectionEntry.SpeedDrink);   // [0.4.5] 图鉴：拾取加速饮料

            // [0.5] 拾取进背包（不再立即加速；使用时 RunManager.ApplyItemEffect）
            var run = RunManager.Instance;
            if (run != null)
            {
                run.AddItem(RunItem.SpeedDrink);
                MissionSystem.OnLootCollected();   // [0.8.0] 每日任务"搜刮"进度
                Debug.Log($"[加速] 拾取进背包 ×{run.GetItemCount(RunItem.SpeedDrink)}（使用时 ×1.4 持续 8s）");
            }
            Destroy(gameObject, 0.3f);
        }
    }
}
